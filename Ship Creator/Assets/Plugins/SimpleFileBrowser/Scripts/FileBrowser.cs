...
#pragma warning restore 0649
        ...
        public void OnSpawnButtonClicked()
        {
            ...
            string filenameInput = filenameInputField.text;
            path = (path + @"\" + filenameInput);
            string toBeSearched = @"Resources\";
            path = path.Substring(path.IndexOf(toBeSearched) + toBeSearched.Length);
            path = path.Substring(0, path.Length - 7);
            Debug.Log(path);
            path = path.Replace(@"\", "/");
            GameObject pPrefab = Resources.Load<GameObject>(path);
            Instantiate(pPrefab, new Vector3(0,5,0), Quaternion.identity);
        }
		...
		public void OnSubmitButtonClicked()
		{
            ...
            Debug.Log(filenameInput.Substring(filenameInput.Length - 7));
            if (filenameInput.Substring(filenameInput.Length - 7) == ".prefab")
            {
                return;
            }
